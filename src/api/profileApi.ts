...
import type { ApiUser, ReplacementMethod } from '../types/api'
...
export async function updateGameName(gameName: string): Promise<ApiUser> {
  ...
  if (!response.ok) {
    const err = await response.text()
    throw new Error(err || `Failed to update user name: ${response.status}`)
  }
...
/**
 * Update user's bench replacement preference
 */
export async function updateReplacementMethod(type: ReplacementMethod): Promise<ApiUser> {
  const token = getToken()
  if (!token) throw new Error('Not authenticated')

  const response = await apiFetch('/api/user/upload', {
    method: 'POST',
    body: JSON.stringify({ name: null, image: null, type }),
  })
