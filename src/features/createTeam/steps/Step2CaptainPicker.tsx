...
import { Loader2, Check, Minus, Sparkles } from 'lucide-react'
    ...
    <div className="flex flex-col h-full">
      ...
      {smartXIPicked && (
        <div className="shrink-0 flex items-center gap-2 px-4 pt-3 pb-1">
          <div className="flex-1 flex items-center gap-2 rounded-lg bg-primary/8 border border-primary/15 px-3 py-2">
            <Sparkles className="h-3.5 w-3.5 text-primary shrink-0" />
            <p className="text-xs text-primary">Smart XI picked a balanced lineup for you. Feel free to swap players or change C/VC before saving.</p>
            {onDismissSmartHint && (
              <button type="button" onClick={onDismissSmartHint} className="ml-auto text-primary/50 hover:text-primary cursor-pointer">
      ...
      <div className="shrink-0 border-t bg-background/95 backdrop-blur-sm px-4 py-3">
        <Button
          ...
          onClick={() => smartXIPicked ? setConfirmOpen(true) : onSave()}
      ...
      <Dialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter className="gap-2 sm:gap-0">
            ...
            <Button onClick={() => { setConfirmOpen(false); onSave() }} className="gap-1.5">
